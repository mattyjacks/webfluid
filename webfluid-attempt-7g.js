...
// Global variables for current color components
var currentR = Math.random();
var currentG = Math.random();
var currentB = Math.random();
...
pointers[0].down = true; // Set the initial pointer as always down
...
function generateColor() {
  // Randomly increment or decrement each color component
  currentR -= (Math.random() - 0.5) * .4; // Increase the rate of change
  currentG -= (Math.random() - 0.5) * .4;
  currentB -= (Math.random() - 0.5) * .4;

  // Ensure each component stays within the 0-1 range
  currentR = Math.max(.2, Math.min(currentR, .7));
  currentG = Math.max(.2, Math.min(currentG, .7));
  currentB = Math.max(0, Math.min(currentB, 1));

  // console.log(`Current Color: R=${currentR}, G=${currentG}, B=${currentB}`); // Debugging log

  // Adjust the intensity of the color
  return {
    r: currentR * .8, // Adjust intensity if needed
    g: currentG * .8,
    b: currentB * 1
  };
}
